...
import com.marctron.transformersmod.entity.ai.AnimationActivateAI;
...
import com.marctron.transformersmod.entity.ai.AnimationDeactivateAI;
...
public class EntityDecepticonBrute extends EntityDecepticon implements IAnimatedEntity {
    ...
    private static final DataParameter<Optional<BlockPos>> REST_POSITION = EntityDataManager.createKey(EntityDecepticonBrute.class, DataSerializers.OPTIONAL_BLOCK_POS);
...
//        tasks.addTask(1, new AnimationDieAI<>(this));
        ...
        tasks.addTask(1, new AnimationAI<EntityDecepticonBrute>(this) {
            {
                setMutexBits(8);
            }
...
//        playSound(MMSounds.ENTITY_WROUGHT_SCREAM, 1f, 1f);
    ...
    protected SoundEvent getAmbientSound() {
        return getAnimation() == NO_ANIMATION && isActive() ? SoundEvents.ENTITY_ENDERDRAGON_HURT : null;
    }
...
//            walkAnim.decreaseTimer(2);
    ...
    private void doVerticalAttackHitFX() {
        ...
        double x = posX + 3. * vecX;
        double y = getEntityBoundingBox().minY + 0.1;
        double z = posZ + 3. * vecZ;
