...
public class EntityHammerPound extends Entity {
    private static final DataParameter<Boolean> VERTICAL = EntityDataManager.createKey(EntityHammerPound.class, DataSerializers.BOOLEAN);
    ...
    public EntityHammerPound(World world) {
        super(world);
    }

    public EntityHammerPound(World world, EntityLivingBase caster, boolean vertical) {
        this(world);
        this.caster = caster;
    ...
    public void onUpdate() {
        ...
        if (!world.isRemote && ticksExisted == 7) ;
            if (!world.isRemote && caster != null) {
                if (!getVertical() && ticksExisted == swingDurationHoriz/2 - 1) dealDamage(7 * 4.5f, 160, 1.2f, quakeAngle );
...
//                    playSound(SoundEvents.BLOCK_ANVIL_LAND, 0.3F, 0.5F);
                    ...
                    playSound(SoundsHandler.HAMMERPOUND, 0.8F, 1.1F);
                ...
                for (int i = 0; i < arcLen; i++) {
                    ...
                    for (Entity entity : hit) {
                        ...
                        if (entity instanceof EntityLivingBase) {
                            if (caster instanceof EntityPlayer) entity.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer) caster), (factor * 5 + 1) );
                            else entity.attackEntityFrom(DamageSource.causeMobDamage(caster), (factor * 5 + 1) );
                        }
