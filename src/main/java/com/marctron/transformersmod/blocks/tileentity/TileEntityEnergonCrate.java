...
public class TileEntityEnergonCrate extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return this.hasCustomName() ? this.customName : "container.energon_crate";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerEnergonCrate)
                {
                    if (((ContainerEnergonCrate)entityplayer.openContainer).getChestInventory() == this)
                    {
