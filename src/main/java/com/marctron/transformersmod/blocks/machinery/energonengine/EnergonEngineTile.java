...
import com.marctron.transformersmod.capabilities.Worker;
...
public class EnergonEngineTile extends TileEntityEnergyProducer implements ITickable {
	 ...
	 * the world)
	 */
	public EnergonEngineTile() {
		super(100000, 0, 1000);
		if (this.world != null)
			this.type = this.world.getBlockState(this.pos).getValue(MachineBlock.TYPE);
	 ...
	 */
	public EnergonEngineTile(ChipTypes type) {
			...
			if (!this.world.isRemote) {
				...
				if (this.storage.getMaxEnergyStored()
					...
					this.world.setBlockState(this.pos,
							this.world.getBlockState(this.pos).withProperty(EnergonEngineBlock.ACTIVATED, true), 2);
				} else {
					this.world.setBlockState(this.pos,
							this.world.getBlockState(this.pos).withProperty(EnergonEngineBlock.ACTIVATED, false), 2);
				}
				if (this.worker.getMaxWork() == 1)
					this.world.setBlockState(this.pos,
							this.world.getBlockState(this.pos).withProperty(EnergonEngineBlock.ACTIVATED, false), 2);
