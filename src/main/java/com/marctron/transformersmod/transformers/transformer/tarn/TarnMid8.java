...
public class TarnMid8 extends ItemArmor implements IHasModel {
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
		player.addPotionEffect(new PotionEffect(MobEffects.INVISIBILITY, 10, 0, bFull3D, false));
			...
			if (itemStack.getItem() == ModItems.TARN_MID8_HELMET) {
				if (world.isRemote) 
				{
			...
			if (itemStack.getItem() == ModItems.TARN_MID8_CHESTPLATE) {
				if (world.isRemote) 
				{
					
						player.inventory.armorInventory.set(2, new ItemStack(ModItems.TARN_CHESTPLATE));
						//player.inventory.armorInventory.set(3, new ItemStack(ModItems.VEHICON_ALTMODE_HELMET));
			...
			if (itemStack.getItem() == ModItems.TARN_MID8_LEGGINGS) {
				if (world.isRemote) 
				{
					
						player.inventory.armorInventory.set(1, new ItemStack(ModItems.TARN_LEGGINGS));
						//player.inventory.armorInventory.set(1, new ItemStack(ModItems.VEHICON_ALTMODE_LEGGINGS));
				
					
				}
			} 
			if (itemStack.getItem() == ModItems.TARN_MID8_BOOTS) {
				if (world.isRemote) 
				{
					
						player.inventory.armorInventory.set(0, new ItemStack(ModItems.TARN_BOOTS));
